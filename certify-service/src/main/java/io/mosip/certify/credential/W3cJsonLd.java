        ...
        if (!signAlgorithm.contains("-")) {
            ...
            com.danubetech.dataintegrity.canonicalizer.Canonicalizer c =
                    com.danubetech.dataintegrity.canonicalizer.Canonicalizers
                            .findDefaultCanonicalizerByAlgorithm("urdna2015");
            byte cano[];
            try {
            ...
            KeymanagerByteSigner k = new KeymanagerByteSigner(signAlgorithm, appID, refID,
                    signatureService);
            try {
                // TODO: replace null with the actual canonicalized byte input
                byte[] output = k.sign(cano, signAlgorithm); // already multibase bas58btc
